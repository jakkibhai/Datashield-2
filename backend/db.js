...
function createStore(filePath) {
  let data = { wipe_sessions: {}, certificates: {}, certificateOrder: [], scanHistory: [] };
  ...
  function load() {
    try {
      ...
      data = { wipe_sessions: {}, certificates: {}, certificateOrder: [], scanHistory: [] };
    }
  }
  ...
  return {
    ...
    // Scan history — every risk scan run this session (sandbox or real
    // folder), kept so the UI can show "did the risk score go down after
    // I cleaned up / wiped?" across a demo without a real database.
    insertScanRecord(record) {
      data.scanHistory.push(record);
      if (data.scanHistory.length > 50) data.scanHistory = data.scanHistory.slice(-50);
      save();
    },
