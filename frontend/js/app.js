...
/* ================= sound engine (Web Audio, synthesized — zero audio files) ================= */
let audioCtx = null;
...
function tone(freq, startOffset, duration, type = "sine", peak = 0.18) {
  ...
  } catch (err) { /* audio unavailable — fail silently, never block the UI */ }
}

const sound = {
  stamp() {
    tone(90, 0, 0.18, "sine", 0.3);            // low thud, like a seal hitting paper
    tone(880, 0.08, 0.22, "triangle", 0.14);   // bright ding
    tone(1320, 0.14, 0.28, "triangle", 0.1);
  },
  success() {
    tone(660, 0, 0.14, "sine", 0.16);
    tone(880, 0.09, 0.22, "sine", 0.16);
  },
  error() {
    tone(220, 0, 0.16, "sawtooth", 0.14);
    tone(150, 0.1, 0.2, "sawtooth", 0.12);
  },
};
...
document.getElementById("sound-toggle").addEventListener("click", () => {
  ...
  if (soundEnabled) { getAudioCtx(); tone(660, 0, 0.1, "sine", 0.12); }
});
...
// Certificate issuance: a wax-seal "stamp" impact — a full-screen flash
// with a gold ring, plus rippling rings expanding out of the card itself.
// Fits the "notarized document" identity far better than confetti does.
function stampCertificate() {
  sound.stamp();
  const overlay = document.createElement("div");
  overlay.className = "cert-stamp-overlay";
  overlay.innerHTML = `
    <div class="cert-stamp-ring"><span class="cert-stamp-glyph">◈</span></div>
    <div class="cert-stamp-text">CERTIFICATE ISSUED</div>
  `;
  document.body.appendChild(overlay);
  setTimeout(() => overlay.classList.add("out"), 650);
...
// Reveals text by scrambling through random hex characters before settling
// on the real value — makes a hash feel like it's actually being computed
// in front of you, instead of just appearing.
...
// Animates the risk ring's SVG arc from empty to `pct`% over `duration`ms,
// in step with the number counting up beside it.
function animateRingArc(circle, pct, duration = 800) {
  const r = 52;
...
async function selectDevice(id, cardEl) {
  ...
  document.getElementById("risk-report").classList.add("hidden");
  document.getElementById("scan-skeleton").classList.remove("hidden");
  ...
  document.getElementById("scan-skeleton").classList.add("hidden");
...
function renderRiskReport(report) {
  ...
  animateRingArc(arc, report.riskScore, 900);
...
/* ---- Risk score history — a small bar chart of every scan this session ---- */
async function loadScanHistory() {
  ...
  if (history.length < 2) { block.classList.add("hidden"); return; }
  ...
  recent.forEach((h, i) => {
    ...
    bar.style.background = colorMap[h.riskLevel] || "var(--ink)";
    ...
    requestAnimationFrame(() => { bar.style.height = `${Math.max(6, h.riskScore)}%`; });
  });
}
...
async function generateCertificate() {
  const skel = document.getElementById("cert-skeleton");
  const card = document.getElementById("certificate-card");
  ...
  if (!res.ok) { skel.classList.add("hidden"); alert(cert.error || "Could not generate certificate"); return; }
  ...
  scrambleReveal(document.getElementById("cert-prev-hash"), cert.prevHash, 700);
  scrambleReveal(document.getElementById("cert-hash"), cert.hash, 1100);
...
document.getElementById("download-pdf-btn").addEventListener("click", () => {
  if (!state.certificateId) return;
  window.location.href = `${API_BASE}/certificate/${encodeURIComponent(state.certificateId)}/pdf`;
});
...
async function loadLedger() {
  const skel = document.getElementById("ledger-skeleton");
  skel.classList.remove("hidden");
